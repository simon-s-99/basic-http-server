...
Task HandleSocket(Socket socket)
{
...
static string GetRequestedFilePath(string requestedFileName)
{
    // this version seemingly only works for codecrafters tests?
    //string filePath = Environment.GetCommandLineArgs()[2] + requestTarget.Split('/').LastOrDefault("*");

    //string envPath = Environment.GetCommandLineArgs()[0];
    string envPath = Environment.GetCommandLineArgs()[2];
